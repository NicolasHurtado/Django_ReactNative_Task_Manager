...
from .constants import DATE_FORMAT, ERROR_MESSAGES, QUERY_PARAMS

@pytest.mark.django_db  # Necesario para acceder a la base de datos
class TestTaskAPI:
    @pytest.fixture(autouse=True)
    def setup(self, authenticated_client, task_factory):
    ...
    def test_create_task(self, user_factory):
        ...
        other_user = user_factory(username='testuser2')
        data = {
            'title': 'New task',
    ...
    def test_update_task(self, task_factory):
        ...
        response = self.client.patch(task_detail_url, updated_data, format='json')
        assert response.status_code == status.HTTP_200_OK
    ...
    def task_for_deletion(self, task_factory):
    ...
    def test_task_filtering(self, setup_filtering_tasks):
        ...
        search_url = f"{self.task_list_url}search/?{QUERY_PARAMS['start_date']}={self.today}&{QUERY_PARAMS['end_date']}={next_week}"
    ...
    def test_invalid_start_date_format(self):
        ...
        search_url = f"{self.task_list_url}search/?{QUERY_PARAMS['start_date']}=01-01-2025&{QUERY_PARAMS['end_date']}={self.tomorrow}"
        response = self.client.get(search_url)
    ...
    def test_invalid_end_date_format(self):
        ...
        search_url = f"{self.task_list_url}search/?{QUERY_PARAMS['start_date']}={self.today}&{QUERY_PARAMS['end_date']}=01/01/2025"
        response = self.client.get(search_url)
    ...
    def test_both_invalid_date_formats(self):
        ...
        search_url = f"{self.task_list_url}search/?{QUERY_PARAMS['start_date']}=2025/01/01&{QUERY_PARAMS['end_date']}=01/01/2025"
        response = self.client.get(search_url)
    ...
    def test_validate_due_date_before_start_date(self):
    ...
    def test_validate_task_overlap(self, task_factory):
    ...
    def test_validate_task_without_due_date_overlap(self, task_factory):
    ...
    def test_update_task_no_overlap_validation(self, task_factory):
